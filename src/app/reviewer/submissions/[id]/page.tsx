...
export default function ReviewerWorkspacePage({ params }: { params: { id: string } }) {
  ...
  return (
    <AppShell>
      <div className="space-y-6 pb-28 max-w-7xl mx-auto animate-fade-in">
        {/* Top Workspace Header */}
        <div className="pb-card p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-3.5">
            ...
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-black text-text-primary tracking-tight mt-0.5">
                Evaluation: {evidence.student_display_name} <span className="text-text-muted font-normal text-sm font-mono">({evidence.student_program})</span>
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="lg:hidden flex bg-canvas rounded-xl p-1 border border-border text-xs font-mono">
              <button
                onClick={() => setActiveTab('evidence')}
                className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                  activeTab === 'evidence' ? 'bg-surface text-text-primary shadow-sm border border-border-bright' : 'text-text-muted'
                }`}
              >
                Evidence
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                  activeTab === 'rubric' ? 'bg-surface text-text-primary shadow-sm border border-border-bright' : 'text-text-muted'
                }`}
              >
            ...
            {lastSaved && (
              <span className="text-[11px] text-text-muted hidden sm:inline-block font-mono bg-canvas px-3 py-1 rounded-full border border-border">
        ...
        {publishSuccess && (
          <div className="bg-success/10 border border-success/30 rounded-2xl p-6 sm:p-7 shadow-sm space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              <Link
                ...
                className="pb-btn-ghost text-xs font-mono"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          >
            <div className="flex items-center justify-between px-1">
              ...
              <span className="pb-badge pb-badge-accent text-[11px] font-mono">
                Revision 1 (Frozen)
              </span>
            </div>
          ...
          >
            ...
            <div className="pb-card p-5 flex items-center justify-between">
              <div>
              ...
              <div className="flex items-center space-x-3">
                <div className="w-28 bg-canvas rounded-full h-2.5 overflow-hidden border border-border p-0.5">
                  <div
              ...
              {criteria.map((criterion) => {
                ...
                return (
                  <RubricCriterionCard
                    ...
                    onRationaleChange={(rat) => handleScoreChange(criterion.id, scores[criterion.id]?.level ?? 0)}
        ...
        <div className="fixed bottom-0 inset-x-0 bg-surface/90 backdrop-blur-xl border-t border-border p-4 shadow-lg z-30">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4">
            ...
            <div className="flex items-center space-x-3 self-end sm:self-center font-mono">
              <button
                ...
                className="pb-btn-ghost text-xs flex items-center space-x-1.5"
              >
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-semibold text-warning bg-warning/10 hover:bg-warning/20 rounded-xl border border-warning/30 transition-colors flex items-center space-x-1.5"
              >
              ...
              <button
                ...
                className="pb-btn-primary text-xs flex items-center space-x-2"
              >
        ...
        {showPublishModal && (
          <div className="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
            <div className="fixed inset-0 bg-black/70 backdrop-blur-md" onClick={() => setShowPublishModal(false)} />
            <div className="flex min-h-screen items-center justify-center p-4">
              <div className="relative bg-surface rounded-3xl border border-border-accent shadow-lg max-w-lg w-full p-6 sm:p-8 space-y-5">
                ...
                <div className="space-y-4 text-xs text-text-secondary leading-relaxed font-mono">
                  <p>
                    You are about to publish your formal assessment for <strong className="text-text-primary">{evidence.student_display_name}</strong> on <strong className="text-text-primary">{evidence.challenge_title}</strong>:
                  </p>

                  <div className="bg-canvas border border-border rounded-xl p-4 space-y-2.5">
                    {criteria.map((crit) => (
                      <div key={crit.id} className="flex justify-between items-center text-xs">
                        <span className="font-semibold text-text-primary">{crit.skill_name}:</span>
                ...
                <div className="flex items-center justify-end space-x-3 pt-3 border-t border-border font-mono">
                  <button
                    onClick={() => setShowPublishModal(false)}
        ...
        {showChangesModal && (
          <div className="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
            <div className="fixed inset-0 bg-black/70 backdrop-blur-md" onClick={() => setShowChangesModal(false)} />
            <div className="flex min-h-screen items-center justify-center p-4">
              <div className="relative bg-surface rounded-2xl border border-border-accent shadow-lg max-w-lg w-full p-6 sm:p-8 space-y-4">
                <div className="flex items-start justify-between border-b border-border pb-4">
                  <div className="flex items-center space-x-2">
                ...
                <form onSubmit={handleRequestChanges} className="space-y-4">
                  <p className="text-xs text-text-secondary leading-relaxed font-mono">
                    Requesting changes moves this submission to <code className="text-warning font-semibold bg-warning/10 px-1.5 py-0.5 rounded border border-warning/20">changes_requested</code>.
                    No skill attainments will be granted. The student will be prompted to submit a new revision.
                  </p>

                  <div>
                    <label className="block text-xs font-bold text-text-primary mb-1.5 font-mono">
                    ...
                    <textarea
                      ...
                      className="w-full text-xs text-text-primary rounded-xl border border-border bg-canvas p-4 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent placeholder:text-text-muted font-mono"
                      required
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-end space-x-3 pt-3 border-t border-border font-mono">
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 text-xs font-bold text-white bg-warning hover:opacity-90 rounded-xl shadow-md transition-all uppercase tracking-wider"
                    >
